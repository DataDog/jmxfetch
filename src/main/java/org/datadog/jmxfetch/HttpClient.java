  ...
  public HttpClient(String host, int port, boolean verify) {
    ...
    if (!verify) {
      try {
        ...
        log.error(
            "Error installing dummyTrustManager. Communications with the Agent will "
                + "be affected. Agent Status will be unreliable and AutoDiscovery of new "
                + "JMX checks will fail. error: ",
            e);
  ...
  public HttpResponse request(String method, String body, String path) {
    ...
    try {
      ...
      if (responseCode < 200 || responseCode >= 300) {
        ...
        response =
            new HttpClient.HttpResponse(responseCode, new InputStreamReader(con.getInputStream()));
      }
