...
public class AppConfig {
  ...
  /**
   * Boolean setting to determine whether to ignore jvm_direct instances. If set to true, other
   * instances will be ignored.
   */
  @Builder.Default private boolean targetDirectInstances = false;

  /**
   * Boolean setting to determine whether internal executors are launched as daemons or not. This is
   * useful when JMXFetch is embedded in a client app, e.g. for the java tracer, so that the client
   * app's exit doesn't block on the termination of these internal threads.
   */
  @Builder.Default private boolean daemon = false;

  /**
   * Boolean setting to determine whether JMXFetch is embedded in a client app, e.g. for the java
   * tracer. This setting is uncoupled from daemon one, even though very similar. This setting is
   * used to reduce number of threads used by assuming the JMX connection will be local.
   */
  ...
  @Builder.Default private Status status = new Status();
