...
public class Instance {
	...
	private final static List<String> SIMPLE_TYPES = Arrays.asList("long", "java.lang.String", "int", "float", "double",
			"java.lang.Double", "java.lang.Float", "java.lang.Integer", "java.lang.Long",
			"java.util.concurrent.atomic.AtomicInteger", "java.util.concurrent.atomic.AtomicLong", "java.lang.Object",
			"java.lang.Boolean", "boolean", "java.lang.Number");
	...
	private HashMap<String, Object> initConfig;
	...
	public Instance(Instance instance, AppConfig appConfig) {
		this(instance.getYaml() != null ? new HashMap<String, Object>(instance.getYaml()) : null,
				instance.getInitConfig() != null ? new HashMap<String, Object>(instance.getInitConfig()) : null,
				instance.getCheckName(), appConfig);
	}

	@SuppressWarnings("unchecked")
	public Instance(HashMap<String, Object> yamlInstance, HashMap<String, Object> initConfig, String checkName,
			AppConfig appConfig) {
		this.appConfig = appConfig;
		this.yaml = yamlInstance != null ? new HashMap<String, Object>(yamlInstance) : null;
		this.initConfig = initConfig != null ? new HashMap<String, Object>(initConfig) : null;
		this.instanceName = (String) yaml.get("name");
		this.tags = (HashMap<String, String>) yaml.get("tags");
		...
		if (this.refreshBeansPeriod == null) {
			this.refreshBeansPeriod = DEFAULT_REFRESH_BEANS_PERIOD; // Make sure
																	// to
																	// refresh
																	// the beans
																	// list
																	// every 10
																	// minutes
			// Useful because sometimes if the application restarts, jmxfetch
			// might read
			// a jmxtree that is not completely initialized and would be missing
			// some attributes
		}
		...
		if (this.instanceName == null) {
			if (this.yaml.get(PROCESS_NAME_REGEX) != null) {
				...
				LOGGER.warn(
						"Cannot determine a unique instance name. Please define a name in your instance configuration");
		...
		// More information:
		// https://issues.apache.org/jira/browse/CASSANDRA-4009
		this.cassandraAliasing = (Boolean) yaml.get("cassandra_aliasing");
		if (this.cassandraAliasing == null) {
			this.cassandraAliasing = false;
		}

		// In case the configuration to match beans is not specified in the
		// "instance" parameter but in the initConfig one
		...
		configurationList.add(new Configuration(
				(HashMap<String, Object>) new YamlParser(this.getClass().getResourceAsStream("/jmx-1.yaml"))
						.getParsedYaml()));
		configurationList.add(new Configuration(
				(HashMap<String, Object>) new YamlParser(this.getClass().getResourceAsStream("/jmx-2.yaml"))
						.getParsedYaml()));
	}
	...
	public LinkedList<HashMap<String, Object>> getMetrics() throws IOException {
		...
		// We can force to refresh the bean list every x seconds in case of
		// ephemeral beans
		// To enable this, a "refresh_beans" parameter must be specified in the
		// yaml config file
		if (this.refreshBeansPeriod != null
				&& (System.currentTimeMillis() - this.lastRefreshTime) / 1000 > this.refreshBeansPeriod) {
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (this.failingAttributes.contains(jmxAttr)) {
					LOGGER.debug("Cannot generate metrics for attribute: " + jmxAttr
			...
			for (MBeanAttributeInfo attributeInfo : attributeInfos) {
				...
				if (metricsCount >= maxReturnedMetrics) {
					...
					if (action.equals(AppConfig.ACTION_COLLECT)) {
						...
						break;
					} else if (!metricReachedDisplayed && !action.equals(AppConfig.ACTION_LIST_COLLECTED)
				...
				if (SIMPLE_TYPES.contains(attributeType)) {
					...
					try {
						LOGGER.debug(ATTRIBUTE + beanName + " : " + attributeInfo + " has an unsupported type: "
								+ attributeType);
				...
				// For each attribute we try it with each configuration to see
				// if there is one that matches
				// If so, we store the attribute so metrics will be collected
				// from it. Otherwise we discard it.
				for (Configuration conf : configurationList) {
					try {
						...
						LOGGER.error("Error while trying to match attributeInfo configuration with the Attribute: "
								+ beanName + " : " + attributeInfo, e);
					}
				}
				if (jmxAttribute.getMatchingConf() == null && (action.equals(AppConfig.ACTION_LIST_EVERYTHING)
						|| action.equals(AppConfig.ACTION_LIST_NOT_MATCHING))) {
	...
	/**
	 * Query and refresh the instance's list of beans. Limit the query scope
	 * when possible on certain actions, and fallback if necessary.
	 */
	...
	HashMap<String, Object> getYaml() {
		return this.yaml;
	}

	HashMap<String, Object> getInitConfig() {
